...
public class MatchEngine {
    ...
    public Result calculateMatchDay(Team home, Team away) {
        ...
        for(int chances = 0; chances < 3; chances ++){
            boolean goalScored = getRandomNumberBetween(homeOffensiveRatingOnDay,20)  > getRandomNumberBetween(awayDefensiveRatingOnDay,20) && homeOffensiveRatingOnDay > getRandomNumberBetween(0,20);
            if(goalScored){
                homeGoals++;
            }
        }
        ...
        for(int chances = 0; chances < 3; chances ++){
            boolean goalScored = getRandomNumberBetween(awayOffensiveRatingOnDay,20)  > getRandomNumberBetween(homeDefensivRatingOnDay,20)  && awayOffensiveRatingOnDay > getRandomNumberBetween(0,20);
            if(goalScored){
                awayGoals++;
            }
        }
